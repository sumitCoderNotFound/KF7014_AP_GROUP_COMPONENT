...
public class MonitorService {
    ...
    private void processAndStoreLine(String line) {
        ...
        try {
            String objectId = values[0].trim().isEmpty() ? null : values[0].trim();
            Optional<WaterQuality> existing = objectId == null
                    ? Optional.empty()
                    : repository.findByObjectId(objectId);

            WaterQuality record = existing.orElse(new WaterQuality());

            record.setTimestamp(LocalDateTime.now());
            record.setObjectId(objectId);
            record.setPh(parseDoubleOrDefault(values[1], "pH", 0.0));
            record.setAlkalinity(parseDoubleOrDefault(values[2], "ALK", 0.0));
            record.setConductivity(parseDoubleOrDefault(values[3], "COND", 0.0));
            record.setBod(parseDoubleOrDefault(values[4], "BOD", 0.0));
            record.setNitrite(parseDoubleOrDefault(values[5], "NO2", 0.0));
            record.setCusol1(parseDoubleOrDefault(values[6], "CUSOL1", 0.0));
            record.setCusol2(parseDoubleOrDefault(values[7], "CUSOL2", 0.0));
            record.setFesol1(parseDoubleOrDefault(values[8], "FESOL1", 0.0));
            record.setZnsol(parseDoubleOrDefault(values[9], "ZNSOL", 0.0));

            repository.save(record);
            logger.info("Stored record: ID={}, ObjectID={}", record.getId(), record.getObjectId());
    ...
    private void validateCsvLine(String[] values) {
        ...
        if (values.length != EXPECTED_CSV_COLUMNS) {
            throw new CsvProcessingException(
                    String.format("Invalid CSV line format. Expected %d columns but found %d", EXPECTED_CSV_COLUMNS, values.length)
            );
        }
    }
    ...
    private double parseDouble(String value, String fieldName) {
        try {
            ...
            throw new CsvProcessingException("Error parsing " + fieldName + " value: " + value);
        }
    }
